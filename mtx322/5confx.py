...
class AnnalyResult(object):
    def __init__(self,yt_labelme,test_labelme,out_path,title_png):
        self.yt_labelme = yt_labelme # ground truth json
        self.test_labelme = test_labelme # predicton json
    ...
    # 若type为polygon，则将polygon转化为box形式
    def get_points_box(self,points,type='polygon',width=2):
        ...
        if type=='circle' and len(points)==2:
            ...
            return box
        # box = [point第0列的最小值，point第1列的最小值，point第0列的最大值，point第1列的最大值]
        box = [min(points[:,0]),min(points[:,1]),max(points[:,0]),max(points[:,1])] # 由box的左上角坐标和右下角坐标确定一个box
        return box
    ...
    def save_json(self,dic,path):
        ...
        return 0
    def compute_iou(self,bbox1, bbox2): # 重要
        """
        compute iou
        :param bbox1:
        :param bbox2:
        ...
        bbox2xmax = bbox2[2]
        bbox2ymax = bbox2[3]
        area1 = (bbox1ymax - bbox1ymin) * (bbox1xmax - bbox1xmin) # area1 = h1*w1 为area1的面积
        area2 = (bbox2ymax - bbox2ymin) * (bbox2xmax - bbox2xmin) # area2 = h2*w2 为area2的面积
        # 交集坐标
        ...
        if bboxymin >= bboxymax:
            return 0
        area = (bboxymax - bboxymin) * (bboxxmax - bboxxmin) # 交集面积
    ...
    def proce_compute(self,input_json,pre_json,save_path): # input_json is GT, pre_json is prediction
